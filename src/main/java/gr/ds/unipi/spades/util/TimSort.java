...
/* https://github.com/openjdk-mirror/jdk7u-jdk/blob/master/src/share/classes/java/util/TimSort.java */
class TimSort {
	/**
     ...
     */
    private final Comparator<? super Point> c;
     ...
     */
    private TimSort(Point[] a, Comparator<? super Point> c) {
        this.a = a;
        this.c = c;
        ...
        Point[] newArray = new Point[len < 2 * INITIAL_TMP_STORAGE_LENGTH ?
    ...
    private static void binarySort(Point[] a, int lo, int hi, int start,
     ...
     *          the specified array
     */
    private static int countRunAndMakeAscending(Point[] a, int lo, int hi,
                                                    Comparator<? super Point> c) {
        assert lo < hi;
        int runHi = lo + 1;
        if (runHi == hi)
     ...
     */
    private static void reverseRange(Point[] a, int lo, int hi) {
        hi--;
        while (lo < hi) {
            Point t = a[lo];
            a[lo] = a[hi];
     ...
     *    should follow it.
     */
    private static int gallopLeft(Point key, Point[] a, int base, int len, int hint,
                                      Comparator<? super Point> c) {
     ...
     * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
     */
    private static int gallopRight(Point key, Point[] a, int base, int len,
     ...
     */
    private Point[] ensureCapacity(int minCapacity) {
