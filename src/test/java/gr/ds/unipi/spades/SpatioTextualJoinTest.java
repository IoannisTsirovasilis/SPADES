...
public class SpatioTextualJoinTest extends TestCase
{
    ...
    public void testOutputsCorrectSpatialPairs() throws NumberFormatException, IOException {
    	...
    	double radius = 400;
    	JavaPairRDD<Integer, List<Point>> groupedPairs = stj.map(points, broadcastQuadTree, radius, keywords, broadcastStj);
    	JavaRDD<Tuple2<Point, Point>> out = stj.reduce(groupedPairs, radius, 2, broadcastStj);
    	System.out.println(out.count());
    	for ( Tuple2<Point, Point> pp : out.collect()) {
    		System.out.print(pp._1.toString() + " - " + ((FeatureObject) pp._2).getTextualRelevanceScore());
    		System.out.println(pp._2.toString());
    	}
    	sc.close();
    }
